...
propósito geral das medições e a identificação de medidas ou métricas que\todo[inline, color=SeaGreen!50]{responder algo a alguém} 
...
quais \todo[inline, color=SeaGreen!50]{uma entidade, a entidade?}entidade é mapeada \cite{ISO:15939}. As escalas podem ser:
...
programação assumissem diversas características entre si. \todo[inline, color=SeaGreen!50]{confuso}O
...
\begin{easylist}
	...
	%--------------------------
	& \textbf{ANPM} (\textit{Average Number of Parameters per Method} - Média do Número de Parâmetros por Método) calcula a média de parâmetros dos métodos da classe. Seu valor mínimo é zero e não existe um limite máximo para o seu resultado, mas um número alto de parâmetros pode indicar que um método pode ter mais \todo[inline, color=SeaGreen!50]{mais uma responsabilidade ou mais de uma responsabilidade}uma responsabilidade \cite{Basili1987}.
